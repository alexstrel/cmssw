			...
			for (int i : uniform_elements(acc,sizeEleSeeds)) 
			{
				...
				for(int j = 0; j < sizeSCs; ++j)
				{
					...
					if(j >= sizeSCs) break;
			 		...
			 		const float x = viewSCs[j].scR() * alpaka::math::sin(acc,viewSCs[j].scSeedTheta()) * alpaka::math::cos(acc,viewSCs[j].scPhi());				
					const float y = viewSCs[j].scR() * alpaka::math::sin(acc,viewSCs[j].scSeedTheta()) * alpaka::math::sin(acc,viewSCs[j].scPhi());
					...
					const float e = viewSCs[j].scEnergy();
			 		...
			 		for (int charge : {1,-1}) 
			 		{
			 			...
			 			const Vec3d position(newfreeTS.position());
			 			const Vec3d momentum(newfreeTS.momentum());
						
			 			auto transverseCurvature = [](const Vec3d& p, int charge, const float& magneticFieldZ) -> float {
   			  				return -2.99792458e-3f * (charge / alpaka::math::sqrt(acc, p[0] * p[0] + p[1] * p[1])) * magneticFieldZ;  
			 			};
			 			...
			 			PlanePortable::Plane<Vec3d> plane(surfPosition,surfRotation);

			 			constexpr float small = 1.e-6; 

			 			auto u = plane.normalVector();
			 			if (alpaka::math::abs(acc,u[2]) < small) {
			 				Propagators::helixBarrelPlaneCrossing<Propagators::PropagatorDirection::oppositeToMomentum>(acc,position,momentum,rho,surfPosition,surfRotation,theSolExists,propagatedPos,propagatedMom,s);
			 			} 
						...
						const double scale = momentum.norm() / propagatedMom.norm();

						for (int i = 0; i < 3; i++) {
						  propagatedMom[i] *= scale;	
						}
						//propagatedMom.normalize(); 
						//propagatedMom*= momentum.norm();

						// Construct relative point-pair struct for applying quality cuts
						EleRelPointPairPortable::EleRelPointPair<Vec3d> pair(hitPosition,propagatedPos,vertex);

						const float dPhiMax = getCutValue(et, 0.05, 20., -0.002);
						const float dRZMax  = getCutValue(et, 9999., 0., 0);
						const float dRZ     = pair.dZ();

						if(eleSeed.hit0detectorID() != 1) dRZ = pair.dPerp(); 

						if ((dPhiMax >= 0 && alpaka::math::abs(acc, pair.dPhi()) > dPhiMax) || (dRZMax >= 0 && alpaka::math::abs(acc,dRZ) > dRZMax))
							continue;
						const double zVertex = getZVtxFromExtrapolation(vertex, hitPosition,positionSC);
						...
						Vec3d position2(firstMatchFreeTraj.position());
						Vec3d momentum2(firstMatchFreeTraj.momentum());
						...
						propagatedMom = Vec3d(0); 
			 			...
			 			PlanePortable::Plane<Vec3d> plane2(surf2Position,surf2Rotation);
			 			u = plane2.normalVector();

			 			if (alpaka::math::abs(acc, u(2)) < small) {
			 				Propagators::helixBarrelPlaneCrossing<Propagators::PropagatorDirection::alongMomentum>(acc,position2,momentum2,rho,surf2Position,surf2Rotation,theSolExists,propagatedPos,propagatedMom,s);
			 			}
						...
						EleRelPointPairPortable::EleRelPointPair<Vec3d> pair2(hit2Position,propagatedPos,vertexUpdated);
						...
						if ((dPhiMax >= 0 && alpaka::math::abs(acc,pair2.dPhi()) > dPhiMax) || (dRZMax >= 0 && alpaka::math::abs(acc,dRZ) > dRZMax))
							continue;	
