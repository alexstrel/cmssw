...
namespace ALPAKA_ACCELERATOR_NAMESPACE {
	...
	namespace Propagators {
		...
		namespace PlaneCrossing2Order {
			...
			template<typename TAcc, PropagationDirection propDir = anyDirection>
			ALPAKA_FN_HOST_ACC ALPAKA_FN_INLINE bool solutionByDirection(	TAcc const& acc,
				...
				if constexpr (propDir == anyDirection) {
					...
					constexpr double propSign = (propDir == alongMomentum) ? 1. : -1.;
		...
		ALPAKA_FN_HOST_ACC ALPAKA_FN_INLINE void helixArbitraryPlaneCrossing2Order(
			...
			//PropagationDirection propDir,// removed!
