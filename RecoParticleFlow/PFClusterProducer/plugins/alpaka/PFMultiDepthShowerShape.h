        ...
        for (auto idx : ::cms::alpakatools::uniform_group_elements(acc, group, nClusters)) {
          ...
          if constexpr (is_cooperative) {
            ...
            while (iter_lane_idx < eff_w_extent) {
              ...
              const reduce_t proc_pfc_energy = warp::shfl_mask(acc, coop_group_mask, pfc_energy, proc_lane_idx, w_extent);
              ...
              const reduce_t phiSum_ = frac_energy * alpaka::math::abs(acc, ::cms::alpakatools::deltaPhi(acc, phi_rh, proc_phi_c));
            ...
            for (int pfrhfrac_idx = pfrhf_offset; pfrhfrac_idx < (pfrhf_offset + pfrhf_size); pfrhfrac_idx++) {
              ...
              const reduce_t phiSum_tmp = (frac_energy) * alpaka::math::abs(acc, ::cms::alpakatools::deltaPhi(acc, phi_rh, phi_c));
