...
namespace ALPAKA_ACCELERATOR_NAMESPACE {
  ...
  void EpilogueTest::apply(std::vector<Queue> &queues,
    ...
    cms::alpakatools::host_buffer<int> stream_id{cms::alpakatools::make_host_buffer<int, Platform>()};
    ...
    for (int i = 0; i < nIters; i++) {
	  ...
	  *stream_id = i*100 + s;	
	  ...
	  auto dev_size_d = cms::alpakatools::make_device_view<int>(queues[s], devClusteringEpilogueArgs[s].view().size());

	  alpaka::memcpy(queues[s], dev_size_d, stream_id);
	  alpaka::wait(queues[s]);
}
//for (int s = 0; s < nStreams; s++) {
//alpaka::wait(queues[s]);
...
for (int s = 0; s < nStreams; s++) { 
alpaka::wait(queues[s]);
printf("Finished intitialization for %u stream iter %u.\n", s, i);
}
for (int s = 0; s < nStreams; s++) {
          alpaka::exec<Acc1D>(queues[s],
                              workDiv,
...
for (int s = 0; s < nStreams; s++) {
alpaka::wait(queues[s]);
printf("Finished main computation for stream %u iter %u.\n", s, i);
}
#if 0
for (int s = 0; s < nStreams; s++) {
          alpaka::exec<Acc1D>(
	...
	//printf("Sync %u\n", s);
      ...
      printf("%u : current wall time %10e\n\n", i, static_cast<double>(std::chrono::duration_cast<std::chrono::microseconds>(wall_diff).count()) / 1e6);
...
printf("Initialize system : %u clusters, %u rechits, %u rechitfracs\n", nClusters, nHits, nFracs);
...
if( rhfracs[j].pfrhIdx >= nHits ) {
printf("Incorrect index detected! %u >= %u\n", rhfracs[j].pfrhIdx, nHits);
}
...
int main(int argc, char **argv) {
  ...
  std::cout << "nHits " << nHits << ", nFracs " << nFracs << " (default " << nClusters * maxHitsPerCluster << ")"<< std::endl;
