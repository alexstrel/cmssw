...
static constexpr int nColdIters = 100;
...
namespace {
  ...
  CaloCellGeometry::CornersMgr s_cornersMgr(4 * 1048576, CaloCellGeometry::k_cornerSize);  //k_cornerSize = 8;
...
namespace ALPAKA_ACCELERATOR_NAMESPACE {
  ...
  class EpilogueTest {
    ...
    const int threads;
  public:
    EpilogueTest(const int nStreams, const int nIters, const int threads) : nStreams(nStreams), nIters(nIters), threads(threads) { }
  ...
  void EpilogueTest::apply(std::vector<Queue> &queues,
    ...
    for (int i = 0; i < nIters; i++) {
      ...
      if constexpr (multiblock) {
        ...
        for (int s = 0; s < nStreams; s++) {
          //auto dev_clustering_epilogue_args = reco::PFMultiDepthECLCCEpilogueArgsDeviceCollection{queues[s], n};
          //dev_clustering_epilogue_args.zeroInitialise(queues[s]);
          //devClusteringEpilogueArgs.emplace_back(std::move(dev_clustering_epilogue_args));
  ...
  void launch_epilogue_test(std::vector<Queue> &queues,
                            ...
                            const int nStreams, const int nIters, const int threadsPerBlock ) {
    ...
    for (int i = 0; i < inClustersNum; i++) {
      ...
      if (rep == cc_root) {
        ...
        if (is_valid_seed == false) {
          nerrors += 1;
          if(verbose) printf("Component seed is not valid for cluster %d!\n", rep);
        } else {
          if (cc_energy < seed_energy) {
            cc_seed = seed;
            cc_energy = seed_energy;
            if(verbose) printf("updated seed for cluster %d\n", rep);
          }
          is_valid_seed = false;
        }
      }
    }
    if(verbose) printf("TOT rh size = %d , seed = %d (energy %f)\n", tot_recFracSize, cc_seed, cc_energy);
  }
...
//1024 199 1; 
  ...
  std::vector<int> cc_roots = {0, 1, 5, 9, 33, 38, 101, 113, 313, 331, 456};
  //std::vector<int> cc_roots = {0, 1, 5, 9};
  ...
  for (auto const &device : devices) {
    ...
    launch_epilogue_test(
        queues, outHostClusters, outHostRecHitFracs, hostClusteringVars, hostClusters, hostRecHits, hostRecHitFracs, nStreams, nIters, threadsPerBlock);
