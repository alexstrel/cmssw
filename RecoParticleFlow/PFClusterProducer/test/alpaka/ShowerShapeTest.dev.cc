...
static constexpr unsigned int nIters = 1000;
static constexpr unsigned int nColdIters = 100;
...
namespace ALPAKA_ACCELERATOR_NAMESPACE {
  ...
  void ShowerShapeTest::apply(std::vector<Queue>& queues,
                              ...
                              const std::vector<reco::PFRecHitFractionDeviceCollection>& pfRecHitFracs,
                              const std::vector<reco::PFRecHitDeviceCollection>& pfRecHit) const {
    uint32_t items = std::is_same_v<Device, alpaka::DevCpu> ? 1 : 768;
    ...
    printf("Wall time: %f sec per iter, %f per stream per iter\n", wall_time / (nIters-nColdIters), wall_time / ((nIters-nColdIters) * nStreams));
  }
...
void runShowerShapesTest(const ::reco::PFClusterCollection& clusters) {
...
int main() {
  ...
  for (auto const& device : devices) {
    ...
    launch_shower_shape_test(queues, hostClusteringVars, hostClusters, hostRecHits, hostRecHitFracs);

    auto nerrors = checkShowerShapes(clusters, hits, hostClusteringVars[0]);
