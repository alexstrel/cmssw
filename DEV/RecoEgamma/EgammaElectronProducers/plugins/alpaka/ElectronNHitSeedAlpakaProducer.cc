				...
				for (int charge : {1,-1}) 
				{
					...
					{			
						...
						if (std::abs(u(2)) < small) {
							...
							Propagators::helixBarrelPlaneCrossing<Propagators::oppositeToMomentum>(testposition,testmomentum,rho,surfPosition,surfRotation,theSolExists,x2,p2,s);
						} 
						else if ((std::abs(u(0)) < small) && (std::abs(u(1)) < small)) 
						{
							// forward plane HelixForwardPlaneCrossing
							Propagators::helixForwardPlaneCrossing<Propagators::oppositeToMomentum>(testposition,testmomentum,rho,plane,s,x2,p2,theSolExists);
						} 
						else {
