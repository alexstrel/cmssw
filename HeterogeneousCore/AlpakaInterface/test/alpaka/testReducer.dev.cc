...
struct AxpyNorm : public blas::TransformReduceFunctor<reduce_t, Transformer, Reducer, site_unroll_> {
  ...
  AxpyNorm(const DataType &a, const DataType &, transformer_t t, reducer_t r)
      : blas::TransformReduceFunctor<reduce_t, Transformer, Reducer, site_unroll_>(t, r), a(a) {}
  AxpyNorm(const DataType &a, const DataType &)
      : blas::TransformReduceFunctor<reduce_t, Transformer, Reducer, site_unroll_>(Transformer(), Reducer()), a(a) {}

  template <typename TAcc, typename TData>
  ALPAKA_FN_ACC inline reduce_t transform(TAcc const &acc,
                                          const cms::alpakatools::VecArray<TData *, nSrc> &x,
  ...
  ALPAKA_FN_ACC inline reduce_t reduce(TAcc const &acc, const reduce_t &x, const reduce_t &y) const {
    return reducer(x, y);
  }
...
public:
  ALPAKA_FN_ACC auto operator()(TAcc const &acc, TransfromReducer tr) const -> void {
    // Thread/Block idx
    auto const batch_idx =
        static_cast<uint32_t>(alpaka::getIdx<alpaka::Block, alpaka::Threads>(acc)[0]);  //the slowest index
...
int main() {
  ...
  for (auto const &device : devices) {
    ...
    using HostViewType = decltype(alpaka::createView(
        std::declval<decltype(devHost)>(), std::declval<DataType *>(), std::declval<Vec2D const &>()));
    ...
    auto max_reduce_blocks =
        2 * alpaka::getAccDevProps<Acc>(devAcc).m_multiProcessorCount;  //only 2 blocks per MP are active
    ...
    auto msrc_axpyNorm_functor =
        instantiateTransformReduceKernel<Acc,
                                         decltype(devAcc),
                                         QueueAcc,
                                         Buf_t,
                                         Buf_t,
                                         ...
                                         func_t,
                                         nSrc,
                                         false>(devAcc, computeQueue, a, a, xAcc, yAcc, yAcc, xAcc);
    ...
    alpaka::exec<Acc>(computeQueue, workDiv, axpyNormProduct, msrc_axpyNorm_functor);
    ...
    std::cout << "Result on the host :: " << std::setprecision(16) << gres << std::endl;
    //
